...
public class ChatCleanupBatch {

    private final GameRoomChatRepository chatRepository;
    private final BatchService batchService;

    public static final String BATCH_ID = "BATCH_CHAT_CLEANUP";
    ...
    public int execute(BatchExecutionHistory.ExecutionType executionType) {
        ...
        try {
            ...
            int deletedCount = chatRepository.deleteOldChats(threshold);

            if (deletedCount > 0) {
                resultMessage.append(String.format("30일 지난 채팅 %d건 삭제.", deletedCount));
            } else {
                resultMessage.append("삭제할 채팅 로그가 없습니다.");
            }
