...
async function editGenre(id) {
    try {
        ...
        if (!response.ok) {
            throw new Error('장르 정보를 불러올 수 없습니다.');
        }
...
async function deleteGenre(id) {
    if (!confirm('정말 삭제하시겠습니까?\n해당 장르를 사용하는 노래가 있으면 삭제되지 않습니다.')) {
        return;
    }

    try {
        const response = await fetch(`/admin/genre/delete/${id}`, {
            method: 'POST'
        });
...
async function toggleStatus(id) {
    try {
        const response = await fetch(`/admin/genre/toggle/${id}`, {
            method: 'POST'
        });
